...
**主要变更**
- ✅ 优化遮罩贴图在手机上的显示：限制宽度，防止超出格子边界
- ✅ 调整事件显示顺序：任务和习惯在活动之后显示，避免跨日活动被任务左右错开
- ✅ 优化事件显示数量：根据屏幕尺寸动态调整，善用垂直空间（小屏幕4-6个，中等屏幕5-7个）
- ✅ 区间重叠处理：重叠的区间自动上下排列，不再叠在一起
- ✅ 弹窗标题格式优化：改为 YYYY/MM/DD 格式（如"2026/02/18"）
- ✅ 任务和区间时间显示优化：全天事件不显示"全天"文字，只显示日期范围

**技术细节**
- 移动端遮罩贴图：小屏幕（≤480px）最大宽度30px，中等屏幕（≤768px）最大宽度40px
- 事件排序逻辑：`getRegularEvents()` 函数按类型分组排序（活动/备忘 → 任务/习惯 → 其他）
- 事件显示数量：`getMaxEvents()` 根据屏幕宽度和高度动态计算
- 区间层级计算：`getIntervalLayers()` 使用贪心算法分配层级，重叠区间上下排列（间距2rem）
- 弹窗时间格式化：任务（type: 2）和区间（type: 4）的全天事件不显示"全天"

**提交记录**
- `b09d7ef` - feat: optimize calendar display - mobile sticker size, event ordering, interval layering, modal improvements
